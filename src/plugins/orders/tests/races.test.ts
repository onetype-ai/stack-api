...
{
    const inside = api.kernel.context("orders", who) as unknown as OrderContext;
...
describe("an order paid from two places at once", () =>
{
    ...
    {
        api = await startApi({ payments: PAYMENTS }, undefined, counting());
...
describe("releasing holds that have run out", () =>
{
    test("leaves another shop's reservations alone", async () =>
    {
        ...
        /* Both are old enough, and the sweep runs on schedule with nobody createCaller. */
        ...
        const row = (listing.body as { orders: { id: string; status: string }[] }).orders.find((order) => order.id === theirs);
...
describe("what a shop is listing", () =>
{
    test("says the same thing in the count and in the rows", async () =>
    {
        ...
        const listing = await api.kernel.handle({ method: "GET", path: "/orders", input: {}, caller: who });
